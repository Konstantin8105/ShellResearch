...
// RC004 - compare regular and unregular mesh
func RC004() {
	...
	for typeOfModel := 0; typeOfModel < 2; typeOfModel++ {
		...
		for indexFE, fe := range typeOfFEs {
			...
			if typeOfModel == 0 {
				fmt.Fprintf(f, "Regular mesh\n")
			} else {
				fmt.Fprintf(f, "Unregular mesh\n")
			}
...

			{
				...
				l.LineStyle.Color = GetColor(float64(indexFE*2+typeOfModel) / float64(len(typeOfFEs)*2.0))

				// Add the plotters to the plot, with a legend
				// entry for each
				p.Add(l)
				if typeOfModel == 0 {
					p.Legend.Add(fmt.Sprintf("Regular model. Finite Element :%v", fe), l)
				} else {
					p.Legend.Add(fmt.Sprintf("Unegular model. Finite Element :%v", fe), l)
				}
			}
			{
				// Make a line plotter and set its style.
				...
				p2.Add(l)
				if typeOfModel == 0 {
					p2.Legend.Add(fmt.Sprintf("Regular model. Finite Element :%v", fe), l)
				} else {
					p2.Legend.Add(fmt.Sprintf("Unegular model. Finite Element :%v", fe), l)
				}
			}
